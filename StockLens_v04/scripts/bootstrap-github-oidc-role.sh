...
usage() {
  cat <<'USAGE'
Usage:
  scripts/bootstrap-github-oidc-role.sh create
  scripts/bootstrap-github-oidc-role.sh destroy
...
role_name="stocklens-v04-github-infra-bootstrap-role"
...
ensure_oidc_provider() {
  ...
  aws "${aws_args[@]}" iam create-open-id-connect-provider \
    ...
    --tags \
      Key=Project,Value=stocklens-v04 \
      Key=Release,Value=stocklens-v04 \
      Key=Tenant,Value=aws-cday-argentina-2026-v4 \
...
create_bootstrap_role() {
  ...
  if aws "${aws_args[@]}" iam get-role --role-name "$role_name" >/dev/null 2>&1; then
    ...
    aws "${aws_args[@]}" iam create-role \
      ...
      --description "Bootstrap role for StockLens v04 Terraform GitHub Actions OIDC" \
      --tags \
        Key=Project,Value=stocklens-v04 \
        Key=Release,Value=stocklens-v04 \
        Key=Tenant,Value=aws-cday-argentina-2026-v4 \
  ...
  echo
  echo "STOCKLENS_V04_INFRA_ROLE_ARN"
  echo "arn:aws:iam::${account_id}:role/${role_name}"
  echo
  echo "After Terraform creates stocklens-v04-github-infra-role, update the secret to:"
  echo
  echo "arn:aws:iam::${account_id}:role/stocklens-v04-github-infra-role"
}
