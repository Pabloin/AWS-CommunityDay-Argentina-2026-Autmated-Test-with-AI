...
locals {
  ...
  front_mobile_bucket_name = var.create_storage_buckets ? aws_s3_bucket.front_mobile[0].bucket : data.aws_s3_bucket.front_mobile[0].bucket
  front_admin_bucket_name  = var.create_storage_buckets ? aws_s3_bucket.front_admin[0].bucket : data.aws_s3_bucket.front_admin[0].bucket
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      },
      {
        ...
        Resource = "${local.photos_bucket_arn}/tenants/${var.default_tenant_id}/*"
      }
    ]
  })
}
...
resource "aws_lambda_function" "api" {
  ...
  environment {
    variables = {
      ...
      PHOTOS_BUCKET     = local.photos_bucket_name
    ...
    Statement = [
      {
        ...
        Resource = [
          local.front_mobile_bucket_arn,
          local.front_admin_bucket_arn
        ]
      },
      {
        Effect = "Allow"
        ...
        Resource = [
          local.mobile_objects_arn,
          local.admin_objects_arn
        ]
      },
      {
        Effect = "Allow"
